...
import {T, weatherEs, terrainEs, natureEs} from './es.js';
...
export function display(
  ...
  const damageText = `${min} - ${max} PS (${minDisplay} - ${maxDisplay}${notation})`;
...
export function getRecovery(
  ...
  const change = recovery[0] > 0 ? 'recuperado' : 'perdido';
...
export function getRecoil(
  ...
  if (move.recoil) {
    ...
    case 2: case 3: case 4:
      if (defender.hasType('Ghost')) {
        if (gen.num === 4) {
          ...
          text = `${gen4CrashDamage}% daño de caída`;
        } else {
          recoil = 0;
          text = 'sin daño de caída en tipos Fantasma';
        }
      } else {
        text = `${minRecoilDamage} - ${maxRecoilDamage}${notation} daño de caída al fallar`;
      }
      break;
    default:
      recoil = notation === '%' ? 24 : 50;
      text = '50% de daño de caída';
    }
  } else if (move.struggleRecoil) {
    recoil = notation === '%' ? 12 : 25;
    text = '25% de daño de forcejeo';
    ...
    if (gen.num === 4) text += ' (redondeado hacia abajo)';
  } else if (move.mindBlownRecoil) {
    recoil = notation === '%' ? 24 : 50;
    text = '50% de daño de retroceso';
  }
...
export function getKOChance(
  ...
  if (damage[0] >= defender.maxHP() && move.timesUsed === 1 && move.timesUsedWithMetronome === 1) {
    return {chance: 1, n: 1, text: 'OHKO garantizado'};
  }
  ...
  const qualifier = approximate ? 'aprox. ' : '';

  const hazardsText = hazards.texts.length > 0
    ? ' tras ' + serializeText(hazards.texts)
    : '';
  const afterText =
    hazards.texts.length > 0 || eot.texts.length > 0
      ? ' tras ' + serializeText(hazards.texts.concat(eot.texts))
      : '';
  const afterTextNoHazards = eot.texts.length > 0 ? ' tras ' + serializeText(eot.texts) : '';
  ...
  function KOChance(
    ...
    if (chanceWithoutEot === undefined || chanceWithEot === undefined) {
      ...
      if (chanceWithEot === 1) {
        text += `${roundChance(chanceWithoutEot)}% de prob. de ${KOTurnText}${hazardsText} ` +
          `(garantizado ${KOTurnText}${afterTextNoHazards})`;
        // if the move OHKOing is possible, and eot damage increases the odds of the KO
      } else if (chanceWithEot > chanceWithoutEot) {
        text += `${roundChance(chanceWithoutEot)}% de prob. de ${KOTurnText}${hazardsText} ` +
          `(${qualifier}${roundChance(chanceWithEot)}% de prob. de ` +
      ...
      if (chanceWithEot === 1) {
        ...
        text += `${roundChance(chanceWithEot)}% de prob. de ${KOTurnText}${afterText}`;
      }
    }
...
function getEndOfTurn(
  ...
  if (field.hasWeather('Sun', 'Harsh Sunshine')) {
    if (defender.hasAbility('Dry Skin', 'Solar Power')) {
      damage -= Math.floor(defender.maxHP() / 8);
      texts.push(T(defender.ability, 'ability') + ' daño');
    }
  } else if (field.hasWeather('Rain', 'Heavy Rain') && !healBlock) {
    if (defender.hasAbility('Dry Skin')) {
      damage += Math.floor(defender.maxHP() / 8);
      texts.push('recuperación Piel Seca');
    } else if (defender.hasAbility('Rain Dish')) {
      damage += Math.floor(defender.maxHP() / 16);
      texts.push('recuperación Cura Lluvia');
    }
  ...
  if (field.attackerSide.isSeeded && !attacker.hasAbility('Magic Guard')) {
    ...
    if (attacker.hasAbility('Liquid Ooze')) {
      damage -= recovery;
      texts.push('daño Lodo Líquido');
    } else if (!healBlock) {
      damage += recovery;
      texts.push('recuperación Drenadoras');
    }
  }
  ...
  if (field.hasTerrain('Grassy')) {
    if (isGrounded(defender, field) && !healBlock) {
      damage += Math.floor(defender.maxHP() / 16);
      texts.push('recuperación Campo Herboso');
    }
  }
  ...
  if (
    ...
    if (attacker.hasItem('Binding Band')) {
      ...
      texts.push('daño de atrapamiento');
    } else {
...
function buildDescription(description: RawDesc, attacker: Pokemon, defender: Pokemon) {
  ...
  output = appendIfSet(output, description.attackerItem && T(description.attackerItem, 'item'));
  output = appendIfSet(output, description.attackerAbility && T(description.attackerAbility, 'ability'));
  output = appendIfSet(output, description.rivalry === 'buffed' ? 'rivalidad fortalecida'
    : description.rivalry === 'nerfed' ? 'rivalidad debilitada' : '');
  if (description.isBurned) {
    output += 'quemado ';
  }
  if (description.alliesFainted) {
    output += Math.min(5, description.alliesFainted) +
      ` ${description.alliesFainted === 1 ? 'aliado debilitado' : 'aliados debilitados'} `;
  }
  ...
  if (description.isBeadsOfRuin) {
    output += 'Cuentas de Ruina ';
  }
  if (description.isSwordOfRuin) {
    output += 'Espada de Ruina ';
  }
  output += T(description.attackerName, 'species') + ' ';
  if (description.isHelpingHand) {
    output += 'Manos Amigas ';
  }
  if (description.isFlowerGiftAttacker) {
    output += 'con Don Floral de un aliado ';
  }
  if (description.isPowerTrickAttacker) {
    output += 'con Truco Fuerza ';
  }
  if (description.isSteelySpiritAttacker) {
    output += 'con Espíritu Acero de un aliado ';
  }
  if (description.isBattery) {
    output += 'Con Batería activa ';
  }
  if (description.isPowerSpot) {
    output += 'Con Enclave Poderoso activo ';
  }
  if (description.isSwitching) {
    output += 'potenciado al cambiar ';
  }
  output += T(description.moveName, 'move') + ' ';
  if (description.moveBP && description.moveType) {
    output += '(' + description.moveBP + ' Pot ' + T(description.moveType, 'type') + ') ';
  } else if (description.moveBP) {
    output += '(' + description.moveBP + ' Pot) ';
  } else if (description.moveType) {
    output += '(' + T(description.moveType, 'type') + ') ';
  }
  if (description.hits) {
  ...
  output = appendIfSet(output, description.defenderItem && T(description.defenderItem, 'item'));
  output = appendIfSet(output, description.defenderAbility && T(description.defenderAbility, 'ability'));
  if (description.isTabletsOfRuin) {
    output += 'Tablillas de Ruina ';
  }
  if (description.isVesselOfRuin) {
    output += 'Vasija de Ruina ';
  }
  ...
  output += T(description.defenderName, 'species');
  if (description.weather && description.terrain) {
    output += ' bajo ' + weatherEs(description.weather) + ' y ' + terrainEs(description.terrain) + '';
  } else if (description.weather) {
    output += ' bajo ' + weatherEs(description.weather);
  } else if (description.terrain) {
    output += ' en ' + terrainEs(description.terrain);
  }
  if (description.isReflect) {
    output += ' a través de Reflejo';
  } else if (description.isLightScreen) {
    output += ' a través de Pantalla Luminosa';
  }
  ...
  if (description.isFriendGuard) {
    output += ' con CompaGuardia de un aliado';
  }
  if (description.isAuroraVeil) {
    output += ' con Velo Aurora de un aliado';
  }
  if (description.isCritical) {
    output += ' en un golpe crítico';
  }
  if (description.isWonderRoom) {
    output += ' en Sala Extraña';
  }
  return output;
}
